...
# debug: show how PowerShell was launched and what path it thinks it's running
...
$SetTarget = Read-Host "Is this a Server or Client? (S/C): "
switch ($SetTarget.ToUpper()) {
    'S' { $Target = 'Server' }
    'C' { $Target = 'Client' }
    default { 
        Write-Host "Invalid input. Expected 'S' or 'C'." -ForegroundColor Red
        exit
    }
}
# Required info
...
if ($Target -eq 'Client') {
    ...
    Write-Host "2. Set TrustedHosts (required for WinRM in workgroup)" -ForegroundColor Yellow
    Write-Host "3. Enable CredSSP Server role (Without it, WinRM will block the delegation)" -ForegroundColor Yellow
    Write-Host "4. Configure NTLM-only delegation (registry edit)" -ForegroundColor Yellow
    Write-Host "5. Configure AllowFreshCredentials" -ForegroundColor Yellow
    Pause
    ...
    # 4. Configure NTLM-only delegation (registry edit)
    Start-Sleep 1
    Write-Host "Executing:" -BackgroundColor DarkBlue -ForegroundColor White -NoNewline
    Write-Host ' New-Item -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\CredentialsDelegation\AllowFreshCredentialsWhenNTLMOnly" -Force' -ForegroundColor DarkGreen
    # New-Item -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\CredentialsDelegation\AllowFreshCredentialsWhenNTLMOnly" -Force

    Start-Sleep 1
    Write-Host "Executing:" -BackgroundColor DarkBlue -ForegroundColor White -NoNewline
    Write-Host ' New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\CredentialsDelegation\AllowFreshCredentialsWhenNTLMOnly" -Name "1" -Value "wsman/$ServerHostname" -PropertyType String -Force' -ForegroundColor DarkGreen
    # New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\CredentialsDelegation\AllowFreshCredentialsWhenNTLMOnly" -Name "1" -Value "wsman/$ServerHostname" -PropertyType String -Force

    # 5. Configure AllowFreshCredentials
    Start-Sleep 1
    Write-Host "Executing:" -BackgroundColor DarkBlue -ForegroundColor White -NoNewline
    ...
    Start-Sleep 1
    Write-Host "Executing:" -BackgroundColor DarkBlue -ForegroundColor White -NoNewline
    Write-Host ' New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\CredentialsDelegation\AllowFreshCredentials" -Name "1" -Value "wsman/$ServerHostname" -PropertyType String -Force' -ForegroundColor DarkGreen
    # New-ItemProperty -Path "HKLM:\SOFTWARE\Policies\Microsoft\Windows\CredentialsDelegation\AllowFreshCredentials" -Name "1" -Value "wsman/$ServerHostname" -PropertyType String -Force
    ...
    Start-Sleep 1
    if (
        [string]::IsNullOrWhiteSpace($ServerIP)       -or
    ...
    # Check step // 4. Configure NTLM-only delegation (registry edit)
    $ClientCheck_NTLM = reg query "HKLM\SOFTWARE\Policies\Microsoft\Windows\CredentialsDelegation\AllowFreshCredentialsWhenNTLMOnly" |
    Select-String -Pattern $ServerHostname -AllMatches | ForEach-Object {foreach($m in $_.Matches){$m.Value}}
    if ($ClientCheck_NTLM -eq $ServerHostname) {$ClientCheck_NTLM_CheckStatus = "Passed"} else {$ClientCheck_NTLM_CheckStatus = "Failed"}
    Start-Sleep 1

    # Check step // 5. Configure AllowFreshCredentials
    ...
    else {Write-Host $ClientCheck_DNS_CheckStatus -ForegroundColor Red}
    # 2. TrustedHosts
    ...
    else {Write-Host $ClientCheck_TrustedHosts_CheckStatus -ForegroundColor Red}
    # 3. CredSSP
    ...
    else {Write-Host $ClientCheck_CredSSP_CheckStatus -ForegroundColor Red}
    # 4. NTLM-only delegation
    Write-Host "| 4. Configure NTLM-only delegation -----: " -NoNewline
    if ($ClientCheck_NTLM_CheckStatus -eq 'Passed') {Write-Host $ClientCheck_NTLM_CheckStatus -ForegroundColor Green}
    else {Write-Host $ClientCheck_NTLM_CheckStatus -ForegroundColor Red}
    # 5. AllowFreshCredentials
    Write-Host "| 5. Configure AllowFreshCredentials ----: " -NoNewline
    if ($ClientCheck_Credentials_CheckStatus -eq 'Passed') {Write-Host $ClientCheck_Credentials_CheckStatus -ForegroundColor Green}
    else {Write-Host $ClientCheck_Credentials_CheckStatus -ForegroundColor Red}
    Write-Host "#======================================#"
    Pause

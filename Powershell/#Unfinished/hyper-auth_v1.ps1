...
if ($Target -eq 'Client') {
    ...
    Write-Host "   - Why: WinRM (SPN) allowed list is Hostname based, not IP" -ForegroundColor Yellow
    Write-Host "2. Set TrustedHosts (permits WinRM connections to non-domain host)" -ForegroundColor Yellow
    Write-Host "   - Why: Host blocks any NTLM WinRM connection, Kerberos is only for AD" -ForegroundColor Yellow
    Write-Host "3. Enable & Configure WinRM Authentication Protocol (CredSSP/SPN)" -ForegroundColor Yellow
    Write-Host "   - Why: Without CredSSP/SPN, WinRM blocks the delegation" -ForegroundColor Yellow
    Write-Host "4. Configure AllowFreshCredentials (allows WinRM to forward to target)" -ForegroundColor Yellow
    Write-Host "   - Why: CredSSP will refuse to forward credentials to client" -ForegroundColor Yellow

...
function Show-RootSystemMSG_Done {
    Write-Host "::System::" -BackgroundColor gray -ForegroundColor magenta -NoNewline
    Write-Host " DONE" -ForegroundColor Green
}
function Show-RootSystemMSG_SUCCESS {
    Write-Host "::System::" -BackgroundColor Gray -ForegroundColor Magenta -NoNewline
    Write-Host " SUCCESS" -ForegroundColor Green
}
function Show-RootSystemMSG_Invalid-Input {
    Write-Host "::System::" -BackgroundColor Gray -ForegroundColor Magenta -NoNewline
    Write-Host " ERROR: Invalid input" -ForegroundColor Red
}
function Show-RootSystemMSG_ERR-Variable-Dependencies {
    Write-Host "::System::" -BackgroundColor Gray -ForegroundColor Magenta -NoNewline
    Write-Host " ERROR: (Variable Dependencies = NUL)" -ForegroundColor Red
}

function Show-RootSystemMSG_ERR-SCRIPT-ERROR {
...
function Get-RootMode {
    ...
    do {
        ...
        switch ($global:SetRootMode) {
            ...
            default {
                Show-RootSystemMSG_Invalid-Input
...
function Get-RootDependencies {
    do {Write-Host "--------------------------" -ForegroundColor Green
        switch ((Read-Host "Is this a Server or Client? (S/C)").ToUpper()) {
            ...
            default 
                {Show-RootSystemMSG_Invalid-Input}}} until ($global:validTarget)
    # Required info
...
function Invoke-ClientSystemCheck {
    ...
    if ([string]::IsNullOrWhiteSpace($global:ServerIP)       -or
       ...
       {Show-RootSystemMSG_ERR-Variable-Dependencies
        ...
        Pause}
    else {Show-RootSystemMSG_SUCCESS}
    ...
    Select-String -Pattern $global:ServerHostname -AllMatches | ForEach-Object {foreach($m in $_.Matches){$m.Value}} | Select-Object -First 1
...
function Get-ServerConfirmation {
    ...
    do {$global:ServerNoticeInput = Read-Host "Press (Enter) to continue / Refresh (R) / Learn step Reason (1-5)"
        ...
        elseif (-not [string]::IsNullOrWhiteSpace($global:ServerNoticeInput)) 
            {Show-RootSystemMSG_Invalid-Input}
    ...
    do {$global:ServerFinalInput = Read-Host "Do you wish to execute? (Enter)"
        if (-not [string]::IsNullOrWhiteSpace($global:ServerFinalInput)) 
        {Show-RootSystemMSG_Invalid-Input}
...
function Invoke-ClientOptionalManualCheck {
    ...
    do {$global:ClientNoticeInput = Read-Host "Press (Enter) to continue / Manual check (1-4)"
        switch ($global:ClientNoticeInput) {
            ...
            '4' { Get-Item WSMan:\localhost\Client\Auth\CredSSP }
            default {
                if (-not [string]::IsNullOrWhiteSpace($global:ClientNoticeInput)) {Show-RootSystemMSG_Invalid-Input}
            }
        }
    } until ([string]::IsNullOrWhiteSpace($global:ClientNoticeInput))
}
...
elseif ($global:RootMode -eq 'Debug') {
    ...
    do {$global:SetDebug = (Read-Host "Press (Enter) to Exit / Select Step (A-E)").ToUpper()
        switch ($global:SetDebug) {
            ...
            default {
                if (-not [string]::IsNullOrWhiteSpace($global:SetDebug)) {Show-RootSystemMSG_Invalid-Input}
            }
        }
...
elseif ($global:RootMode -eq 'Configure') {
    ...
    if ($global:Target -eq 'Client') {
        try {
            ...
            Show-RootSystemMSG_ERR-SCRIPT-ERROR
            Write-Host "The script will now exit"
            Pause
            Exit
        }

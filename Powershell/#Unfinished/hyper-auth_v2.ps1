...
do {
    ...
    switch ((Read-Host "Is this a Server or Client? (S/C)").ToUpper()) {
        ...
        default   { Write-Host "Invalid input. Expected 'S', 'C', 'Server', or 'Client'." -ForegroundColor Red; $validTarget = $false }
    }
} until ($validTarget)
...
if ($Target -eq 'Client') {
    ...
    Write-Host "#--------------------------------------#"
    # Notice prompt (Step descriptions and reasons)
    $NoticeSteps = @{
        '1' = "Why: WinRM (SPN) allowed list is Hostname based, not IP"
        '2' = "Why: Host blocks any NTLM WinRM connection, Kerberos is only for AD"
        '3' = "Why: Without CredSSP/SPN, WinRM blocks the delegation"
        '4' = "Why: CredSSP will refuse to forward credentials to client"}
    ...
    do {
        $NoticeInput = Read-Host "Press Enter to continue, or type step number (1-4) to learn why"
        if ($NoticeInput -match '^[1-4]$') {
            Write-Host $NoticeSteps[$NoticeInput] -ForegroundColor Cyan}
    } until ([string]::IsNullOrWhiteSpace($NoticeInput))

...
def start_monitor_mode():
    ...
    monitor_command = f'airmon-ng check kill {wifi_interface} && airmon-ng start {wifi_interface} 1'
...
def stop_monitor_mode():
    ...
    status_command = "systemctl status NetworkManager | grep Active| sed 's/\(.*\)/\x1b[1;35m\1\x1b[0m/'"
    ...
    if "inactive" in status_output:
        status_output = status_output.replace("inactive (dead)", f"{bcolors.LIGHT_PURPLE}inactive (dead){bcolors.ENDC}")
    print(status_output)
